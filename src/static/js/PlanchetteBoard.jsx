...
export default function PlanchetteBoard() {
  ...
  const exportPDF = useCallback(async () => {
    ...
    const element = el(Document, null, el(Page, { size: "A4", style: s.page },
      logoB64 ? el(View, { style: s.logoWrap }, el(Image, { src: logoB64, style: s.logo })) : null,
      el(Text, { style: s.title }, "PLANCHETTE"),
      el(Text, { style: s.subtitle }, "THE TALKING BOARD \u2014 SESSION LOG"),
      el(Text, { style: s.edition }, "SELF-HOSTED EDITION"),
      el(Text, { style: s.meta }, `Date: ${date}`),
      el(Text, { style: s.meta }, `Disclaimer accepted: ${acceptedStr}`),
      el(View, { style: s.hr }),
      ...log.map((entry, i) =>
        entry.role === "user"
          ? el(View, { key: i, style: s.userRow }, el(Text, { style: s.userLabel }, "You: "), el(Text, { style: s.userText }, entry.text))
          : el(View, { key: i, style: s.spiritRow }, el(Text, { style: s.spiritLabel }, "Spirit: "), el(Text, { style: s.spiritText }, entry.text))
      ),
      el(View, { style: s.hr }),
      el(Text, { style: s.footer }, "Generated by Planchette \u2014 AI Spirit Talking Board"),
      badgeB64 ? el(View, { style: s.badgeWrap }, el(Link, { src: "https://apps.apple.com/us/app/planchette-the-talking-board/id6759858464" }, el(Image, { src: badgeB64, style: s.badge }))) : null
    ));
